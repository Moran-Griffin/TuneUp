...
export default function LogScreen() {
  ...
  return (
    <View className="flex-1 bg-gray-50 dark:bg-black">
      <View className="bg-white dark:bg-[#1c1c1e] pt-16 pb-3 px-4 border-b border-gray-100 dark:border-[#2c2c2e]">
        <View className="flex-row items-center justify-between mb-3">
          <Text className="text-2xl font-bold dark:text-white">Service Log</Text>
          <TouchableOpacity onPress={() => router.push(filter === 'all' ? '/log/new' : `/log/new?type=${filter}`)}>
            <Plus size={24} color={colorScheme === 'dark' ? '#0a84ff' : '#2563eb'} />
          </TouchableOpacity>
        </View>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {filterOptions.map(f => (
            <TouchableOpacity
              key={f}
              className={`mr-2 px-3 py-1 rounded-full ${filter === f ? 'bg-blue-600' : 'bg-gray-100 dark:bg-[#2c2c2e]'}`}
              onPress={() => setFilter(f)}
            >
              <Text className={`text-sm ${filter === f ? 'text-white font-medium' : 'text-gray-600 dark:text-[#8e8e93]'}`}>
      ...
      {loading ? (
        <View className="flex-1 items-center justify-center">
          <Text className="text-gray-400 dark:text-[#636366]">Loading...</Text>
        </View>
      ) : filtered.length === 0 ? (
        <View className="flex-1 items-center justify-center px-8">
          <Text className="text-gray-400 dark:text-[#636366] text-center">
